...
> 本仓库是基于 [geekgeekrun/geekgeekrun](https://github.com/geekgeekrun/geekgeekrun) 的 Fork 版本，在原版基础上增加了以下增强功能：

### 🏙️ 多城市遍历策略

原版在选择多个城市时，每次都从第一个城市开始刷，无法轮换。Fork 版本支持配置多城市遍历方式，提供三种策略：

| 策略 | 说明 |
|------|------|
| **逐个城市刷完**（默认） | 先刷完一个城市的所有职位，再切换到下一个城市 |
| **一轮一个城市轮换** | 每一轮只刷一个城市，下一轮自动切换到下一个城市，循环轮换（跨会话持久化） |
| **按比重随机分配** | 为每个城市设置权重值，按权重概率随机选择下一个刷新的城市 |

该选项仅在选择了 2 个及以上城市时显示，位于城市选择器下方。

<img width="600" alt="多城市遍历策略配置界面" src="./docs/images/multi_city_config_ui.png" />

### 📊 运行状态实时日志

运行过程中在控制台输出详细的城市轮换状态信息，方便了解当前运行情况：

- 当前阶段（初始化 / 切换城市 / 本轮结束 / 新一轮开始）
- 休息时长（秒）
- 当前城市与下一个城市
- 当前使用的策略名称
- 各城市累计访问次数统计
...
<img width="600" alt="城市轮换状态日志示例" src="./docs/images/city_rotation_log.png" />

### 📦 配置导入导出

支持配置的导入和导出功能，方便备份和迁移设置。

### 📈 今日开聊次数统计

在页面侧边栏和运行界面实时显示今日已开聊次数，一目了然。
